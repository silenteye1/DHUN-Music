...
internal class AnalyticsRepositoryImpl(
    ...
    override suspend fun getPeriodStats(
        ...
        withContext(Dispatchers.IO) {
            ...
            coroutineScope {
                val distinctTracksDeferred = async { analyticsDatasource.getDistinctTrackCountInRange(startTimestamp, endTimestamp) }
                val distinctAlbumsDeferred = async { analyticsDatasource.getDistinctAlbumCountInRange(startTimestamp, endTimestamp) }
                val distinctArtistsDeferred = async { analyticsDatasource.getDistinctArtistCountInRange(startTimestamp, endTimestamp) }
                val newArtistsDeferred = async { analyticsDatasource.getNewArtistCountInRange(startTimestamp, endTimestamp) }
                val artistPlaysDeferred = async { analyticsDatasource.getArtistPlayCountsInRange(startTimestamp, endTimestamp) }
                val decadesDeferred = async { analyticsDatasource.getDecadeCountsInRange(startTimestamp, endTimestamp) }
                val datedPlaysDeferred = async { analyticsDatasource.getDatedPlayCountInRange(startTimestamp, endTimestamp) }

                val distinctTracks = distinctTracksDeferred.await()
                val distinctAlbums = distinctAlbumsDeferred.await()
                val distinctArtists = distinctArtistsDeferred.await()
                val newArtists = newArtistsDeferred.await()
                val artistPlays = artistPlaysDeferred.await()
                val decades = decadesDeferred.await()
                val datedPlays = datedPlaysDeferred.await()
